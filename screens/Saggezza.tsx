  ...
  const requestNotificationPermission = async () => {
    ...
    try {
      ...
      if (permission === 'granted') {
        ...
        setNotificationError("Hai negato i permessi. Per attivarli, vai nelle impostazioni del tuo browser.");
      }
    } catch (err) {
      console.error("Errore notifiche:", err);
      setNotificationError("Errore durante l'attivazione.");
    }
  };
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-10">
        ...
        {activeTab === 'CONFIG' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            ...
            <div className="glass-panel p-6 rounded-[2.5rem] border border-white/10">
              <h3 className="text-[10px] font-display font-bold uppercase tracking-[0.3em] text-white/40 mb-6 italic">Promemoria Push</h3>
              ...
              >
                <span className="font-display font-bold text-[10px] uppercase tracking-widest text-white/80 group-hover:dynamic-accent transition-colors">
                  Attiva Notifiche
                </span>
                <div className={`w-12 h-6 rounded-full relative flex items-center px-1 border transition-all ${notificationsEnabled ? 'bg-neon-lime/20 border-neon-lime/50' : 'bg-white/5 border-white/10'}`}>
                  <div className={`w-4 h-4 rounded-full transition-all ${notificationsEnabled ? 'translate-x-6 dynamic-bg shadow-[0_0_10px_currentColor]' : 'translate-x-0 bg-white/20'}`}></div>
                </div>
              </button>
