          ...
          <div className="flex flex-col gap-6">
            ...
            {selectedDate && (
              <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="glass-panel p-6 rounded-[2.5rem] border border-white/10 bg-gradient-to-br from-white/[0.02] to-transparent">
                  <div className="flex justify-between items-center mb-6">
                    <h4 className="text-xs font-display font-black uppercase tracking-[0.2em] text-neon-cyan">
                      DETTAGLIO: {selectedDate}
                    </h4>
                    <button onClick={() => setSelectedDate(null)} className="h-8 w-8 rounded-full bg-white/5 flex items-center justify-center">
                  ...
                  {logs[selectedDate] ? (
                    <div className="space-y-4">
                      <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${logs[selectedDate].amCompleted ? 'bg-neon-lime text-black' : 'bg-white/5 text-white/20'}`}>
                          <span className="material-symbols-outlined">wb_sunny</span>
                        </div>
                        <div>
                          <p className="text-[10px] font-display font-black text-white/40 uppercase">Mattino</p>
                          <p className="text-xs font-bold text-white uppercase italic">
                            {logs[selectedDate].amCompleted ? 'COMPLETATO' : 'NON ESEGUITO'}
                          </p>
                        </div>
                      </div>

                      <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${logs[selectedDate].pmCompleted ? `${NIGHT_THEMES[logs[selectedDate].nightType].color} text-black` : 'bg-white/5 text-white/20'}`}>
                          <span className="material-symbols-outlined">nights_stay</span>
                        </div>
                        <div className="flex-1">
                          <p className="text-[10px] font-display font-black text-white/40 uppercase">Notte {logs[selectedDate].nightType}</p>
                          <p className="text-xs font-bold text-white uppercase italic">
                            {logs[selectedDate].pmCompleted ? NIGHT_THEMES[logs[selectedDate].nightType].label : 'NON ESEGUITO'}
                          </p>
                        </div>
                      </div>
                      ...
                      {logs[selectedDate].notes && (
                        <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                          <p className="text-[10px] font-display font-black text-white/40 uppercase mb-2">Note Operative</p>
                          <p className="text-xs text-white/80 italic leading-relaxed">"{logs[selectedDate].notes}"</p>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="py-8 text-center bg-white/5 rounded-2xl border border-white/5 border-dashed">
                      <span className="material-symbols-outlined text-white/10 text-4xl mb-2">event_busy</span>
                      <p className="text-[10px] font-display font-black text-white/20 uppercase tracking-widest italic">Nessun dato registrato</p>
                    </div>
                  )}
                </div>
              </div>
