...
const CalendarView: React.FC<CalendarViewProps> = ({ logs, onDateSelect, selectedDate, onBack }) => {
  ...
  const completedCount = Object.values(logs).filter((l: DailyLog) => {
    const logDate = new Date(l.date);
    return logDate.getMonth() === viewDate.getMonth() &&
      logDate.getFullYear() === viewDate.getFullYear() &&
      l.amCompleted && l.pmCompleted;
  }).length;
      ...
      <div className="glass-panel p-6 rounded-[2.5rem] mb-8 border border-white/10 shadow-2xl">
        <div className="flex items-center justify-between mb-8">
          <button
            ...
            className="h-10 w-10 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 active:scale-90 transition-all"
          >
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-xl flex items-center justify-center text-xs font-display font-bold transition-all relative
                  ...
                  ${isSelected ? 'scale-110 ring-2 ring-white z-20 shadow-[0_0_20px_rgba(255,255,255,0.4)]' : ''}
                `}
              >
                {d}
